...
from utils.visualization import analyze_role_contrast_loss, visualize_role_embeddings
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(args.output_dir, "{}_run.log".format(time.strftime("%m-%d %H-%M-%S", time.localtime()))),
                mode="w",
                encoding="utf-8",
            )
        ],
    ...
    my_config = {
        ...
        "use_role_contrast": args.use_role_contrast,
        "role_contrast_weight": args.role_contrast_weight,
        "role_contrast_temperature": args.role_contrast_temperature,
        "role_contrast_margin": args.role_contrast_margin
    }
    ...
    for epoch in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                if args.logging_steps > 0 and global_steps % args.logging_steps == 0:
                    ...
                    # 记录角色对比损失相关信息（如果启用）
                    if args.use_role_contrast and loss_info:
                        for k, v in loss_info.items():
                            writer.add_scalar(f"loss/{k}", v, global_steps)
                    
                    log_message = "global_steps {} - lr: {:.8f}  loss: {:.8f}".format(
                        global_steps,
                    ...
                    # 在日志中添加角色对比损失信息（如果启用）
                    if args.use_role_contrast and loss_info:
                        log_message += " role_contrast_loss: {:.8f} weight: {:.8f}".format(
                            loss_info.get("role_contrast_loss", 0),
                            loss_info.get("dynamic_weight", 0)
                        )
                    
                    logger.info(log_message)
                    accelerator.print(log_message)
        ...
        if (
            ...
            if final_f1 >= max_final_f1:
                ...
                savefile = Path(args.output_dir) / "Final_f1_results_{}_{}{}.txt".format(
                    args.contrastive_method,
                    args.description_loss_weight,
                    "_role_contrast" if args.use_role_contrast else ""
                )
                savefile.write_text(
                    pformat(dev_metric), encoding="utf-8")
                
                # 如果启用角色对比损失，在验证集上进行角色嵌入可视化
                if args.use_role_contrast:
                    try:
                        # 获取验证集上的角色嵌入
                        from utils.role_contrast_example import get_role_embeddings
                        vis_dir = os.path.join(args.output_dir, "visualizations")
                        os.makedirs(vis_dir, exist_ok=True)
                        
                        # 获取角色嵌入并进行可视化
                        event_proxy, event_types, role_names = get_role_embeddings(model, dev_dataloader, args)
                        ...
                        logger.info(f"Epoch {epoch+1} Role Contrast Analysis:")
                        for k, v in stats.items():
                            logger.info(f"  {k}: {v:.4f}")
                    except Exception as e:
                        logger.warning(f"角色嵌入可视化失败: {str(e)}")
